...
class page_output
{
	...
	{
		// init
		$this->obj_cdr_rate_table	= New cdr_rate_table_rates;
		$this->obj_service		= New service;


		// fetch key service details
		$this->obj_service->id			= @security_script_input('/^[0-9]*$/', $_GET["id"]);
	...
	function execute()
	{
		...
		$structure["type"]		= "hidden";
		$structure["defaultvalue"]	= $this->obj_cdr_rate_table->id_rate;
		$this->obj_form->add_input($structure);
		...
		$this->obj_form->subforms["hidden"]		= array("id_service", "id_rate");
		...
		else
		{
			...
			$this->obj_cdr_rate_table->option_id		= $this->obj_service->id;

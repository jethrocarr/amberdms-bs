<?php
/*
	services/journal-edit-process.php
...
if (user_permissions_get('services_write'))
{
	...
	$journal->prepare_set_journalname("services");
	...
	// make sure the services ID submitted really exists
	$sql_obj		= New sql_query;
	$sql_obj->string	= "SELECT id FROM services WHERE id='". $journal->structure["customid"] ."'";
	$sql_obj->execute();
	
	if (!$sql_obj->num_rows())
	{
		$_SESSION["error"]["message"][]	= "Unable to find requested service record to modify journal for.";
	}
	...
	{	
		...
		header("Location: ../index.php?page=services/journal.php&id=". $journal->structure["customid"] ."&journalid=". $journal->structure["id"] ."&action=". $journal->structure["action"] ."");
		exit(0);
	}
	else
	{
		...
		header("Location: ../index.php?page=services/journal.php&id=". $journal->structure["customid"] ."");
		exit(0);
	}

<?php
/*
	accounts/ap/credit-journal-download-process.php
	
	access: accounts_ap_view (read-only)

	Allows the download of a file attached to the credit journal.
*/
	...
	*/
	...
	else
	{
		...
		else
		{
			...
			$invoiceid = sql_get_singlevalue("SELECT customid as value FROM journal WHERE journalname='account_ap_credit' AND id='$journalid'");

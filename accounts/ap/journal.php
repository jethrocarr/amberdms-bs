<?php
/*
	accounts/a[/journal.php
	
	access: accounts_ap_view (read-only)
		accounts_ap_write (write access)
...
if (user_permissions_get('accounts_ap_view'))
{
	...
	$_SESSION["nav"]["current"]	= "page=accounts/ap/journal.php&id=$id";
	...
	function page_render()
	{
		...
		*/
		...
		print "<p><b><a href=\"index.php?page=accounts/ap/journal-edit.php&type=text&id=$id\">Add new journal entry</a> || <a href=\"index.php?page=accounts/ap/journal-edit.php&type=file&id=$id\">Upload File</a></b></p>";


		// make sure the invoice exists
		$sql = New sql_query;
		$sql->string = "SELECT id FROM `account_ap` WHERE id='$id'";
		$sql->execute();
		
		if (!$sql->num_rows())
		{
			print "<p><b>Error: The requested invoice does not exist. <a href=\"index.php?page=accounts/ap.php\">Try looking for your invoice on the invoice list page.</a></b></p>";
		}
		else
		{
			/*
			...
			*/
			...
			$journal->journalname	= "account_ap";
			
			// set the pages to use for forms or file downloads
			$journal->prepare_set_form_process_page("accounts/ap/journal-edit.php");
			$journal->prepare_set_download_page("accounts/ap/journal-download-process.php");

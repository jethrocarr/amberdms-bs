...
if (user_permissions_get('accounts_ar_write'))
{
	...
	$sql_obj->string	= "SELECT id FROM account_ar WHERE id='". $journal->structure["customid"] ."'";
	$sql_obj->execute();
	...
	{	
		...
		header("Location: ../../index.php?page=accounts/ar/journal.php&id=". $journal->structure["customid"] ."&journalid=". $journal->structure["id"] ."&action=". $journal->structure["action"] ."");
		exit(0);
	}
	else
	{
		...
		header("Location: ../../index.php?page=accounts/ar/journal.php&id=". $journal->structure["customid"] ."");
		exit(0);
	}

<?php
/*
	support/journal-download-process.php
...
if (user_permissions_get('support_view'))
{
	...
	*/
	...
	else
	{
		...
		{
			// make sure the journal entry belongs to a support
			$supportid = sql_get_singlevalue("SELECT customid as value FROM journal WHERE journalname='support_tickets' AND id='$journalid'");

			if (!$supportid)
			{
				$_SESSION["error"]["message"][] = "Unable to match the provided journal entry to the support_tickets journal.";
			}
		}
	}
